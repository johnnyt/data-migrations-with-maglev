class Generate
  ...
  def self.name_for(gender=nil)
    first_names = case gender
                  ...
                  when :female
                    ...
                    rand(2).zero? ? MALE_NAMES : FEMALE_NAMES
                  end
    [ first_names.at_random, SURNAMES.at_random ]
  end
  ...
  def self.zips
    @zips ||= begin
      zips = []
      FasterCSV.foreach(File.expand_path('../data/zips.csv', File.dirname(__FILE__)), :headers => true) do |row|
        zips << row['ZipCode']
      end
      zips
    end
